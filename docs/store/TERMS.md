# LocalPoker Terms of Service Draft

**App:** LocalPoker: Poker with Friends (`com.localpoker.app`)  
**Effective date:** [PLACEHOLDER: effective date]  
**Publisher:** [PLACEHOLDER: company/legal entity name]  
**Contact:** [PLACEHOLDER: support/legal contact email]  
**Governing law / venue:** [PLACEHOLDER: jurisdiction]

> Legal review required: this is a product-specific draft prepared from the current codebase. It is not legal advice and must be reviewed by a qualified professional before publication.

## 1. Acceptance

By using LocalPoker, you agree to these Terms and the Privacy Policy. If you do not agree, do not use the app.

## 2. Eligibility: adults only

LocalPoker is for users who are **18 years of age or older**. The app uses a blocking birth-year age gate before login. You may not use LocalPoker if you are under 18 or if your local laws prohibit simulated-gambling games.

## 3. Play-money entertainment only

LocalPoker is a free, play-money Texas Hold'em game for entertainment.

LocalPoker does **not** offer:

- real-money gambling;
- cash prizes or prizes with real-world value;
- deposits, withdrawals, cash-outs, or redemption;
- wagering of money or anything of value;
- marketplaces, transfers, or sales of virtual currency or virtual items.

Playing or winning in LocalPoker does not imply skill or success in real-money gambling.

## 4. Virtual coins and cosmetics

Virtual coins, XP, stats, avatars, cosmetics, saved games, and other virtual items are part of the play-money game experience only. They have **no monetary value**, are not property, are not redeemable for cash or value, and are not transferable outside features we expressly provide.

We may change, reset, rebalance, revoke, or discontinue virtual coins/items if needed for gameplay, security, legal compliance, or service operation. In the current build, coins and cosmetics are stored primarily on the device and can be lost if app data is deleted.

## 5. Account and identity

The current build supports guest play, local email-or-username labeling, and anonymous Firebase authentication for online room infrastructure when Firebase is configured. The current email option is not a verified email login, and real Apple/Google/OAuth login is not yet implemented.

You are responsible for activity from your app profile or anonymous account. Do not impersonate others or choose display names that are unlawful, misleading, hateful, harassing, or infringing.

## 6. Friends, rooms, and user content

LocalPoker includes private room codes, display names, Pal/avatar choices, gameplay actions, and local table reactions such as emojis, canned text, custom short text, stickers, and curated GIFs. Firebase-backed friend rooms can sync private room presence and gameplay state when production Firebase setup is enabled.

You agree not to submit or share content that is abusive, harassing, threatening, hateful, sexually explicit, unlawful, infringing, deceptive, spam, or otherwise harmful. Do not share a private room code with anyone you do not want to join.

We may remove content, restrict access, or terminate use if needed to enforce these Terms or protect users.

## 7. Acceptable use

You may not:

- use LocalPoker for real-money gambling, side bets, cash prizes, or value transfer;
- sell, buy, trade, transfer, or redeem virtual coins/items for real-world value;
- cheat, exploit bugs, manipulate client storage, automate gameplay, or disrupt the service;
- reverse engineer the app except where applicable law allows;
- access Firebase or other backend services except through the app;
- overload, scrape, attack, or interfere with the app or third-party services;
- violate law or anyone else's rights.

## 8. Ads and third-party services

Current ad placements are placeholders. **Conditional on ads shipping**, LocalPoker may show ads through AdMob or another ad partner, subject to consent and platform requirements.

LocalPoker also uses Firebase for online infrastructure, Giphy's public CDN for curated GIF images, and optional Sentry diagnostics if enabled. Third-party services have their own terms and privacy practices.

## 9. Updates and availability

We may update, suspend, or discontinue any part of LocalPoker at any time. Features may change before or after store submission. We are not responsible for loss of local progress caused by device loss, app deletion, storage clearing, bugs, or service changes.

## 10. Termination

You may stop using LocalPoker at any time. We may suspend or terminate access, delete data, or restrict features if we believe you violated these Terms, created legal/security risk, or abused the service.

Sections that by nature should survive termination continue to apply, including virtual currency restrictions, acceptable use, disclaimers, liability limits, and governing law.

## 11. Intellectual property

LocalPoker, including its software, artwork, UI, game assets, and trademarks, is owned by [PLACEHOLDER: company/legal entity name] or its licensors. You receive a limited, revocable, non-exclusive, non-transferable license to use the app for personal entertainment according to these Terms.

You retain rights you have in your own display names or submitted content, but you grant us a worldwide, non-exclusive, royalty-free license to host, display, transmit, moderate, and use that content to operate and improve LocalPoker.

## 12. Disclaimers

LocalPoker is provided **as is** and **as available**. To the fullest extent allowed by law, we disclaim warranties of merchantability, fitness for a particular purpose, non-infringement, availability, accuracy, and uninterrupted or error-free operation.

No LocalPoker feature creates gambling, betting, financial, investment, or professional advice.

## 13. Limitation of liability

To the fullest extent allowed by law, [PLACEHOLDER: company/legal entity name] will not be liable for indirect, incidental, special, consequential, exemplary, or punitive damages, or for lost data, lost virtual items, lost profits, or service interruption. Our total liability will not exceed [PLACEHOLDER: liability cap] or the minimum amount required by applicable law.

Some jurisdictions do not allow certain limits, so some limits may not apply.

## 14. Indemnity

To the extent allowed by law, you agree to defend and indemnify [PLACEHOLDER: company/legal entity name] from claims arising out of your misuse of LocalPoker, violation of these Terms, unlawful conduct, or infringement of others' rights.

## 15. Governing law and disputes

These Terms are governed by [PLACEHOLDER: governing law]. Disputes will be resolved in [PLACEHOLDER: courts/arbitration venue], unless applicable consumer law requires otherwise.

## 16. Changes

We may update these Terms. Material changes should be reflected in the hosted terms URL, in-app legal copy, and store metadata before submission.

## 17. Contact

[PLACEHOLDER: company/legal entity name]  
[PLACEHOLDER: mailing address]  
[PLACEHOLDER: support/legal contact email]
